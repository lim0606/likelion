...
class TwitterForm(Form):
    ...
    # 'request_photo' is an argument with the form, 'request.files['photo']'
    def initPhoto(self, request_photo):
        filestream = request_photo.read()
        self.photo.data = db.Blob(filestream)

    # 'request_photo' is an argument with the form, 'request.forms['text']''
    def initMessage(self, request_text):
        self.message.data = db.Text(request_text)
...
"""
    st_text = request.forms['text']
    st = Status()  # declare Photo class type variable
    st.status = db.Text(st_text)
    st.put()  # 'put()' function is defined in BlobProperty class.

    return render_template("upload.html", url=url, text_value=st.status)
"""
